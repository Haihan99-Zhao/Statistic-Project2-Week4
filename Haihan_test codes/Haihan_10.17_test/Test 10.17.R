...
deconv <- function(t, deaths, n.rep=100, bs = FALSE, t0 = NULL){
  ...
  # victims in "t" day" as input. 
  # If we input [t0]: "the estimated incidence trajectory" as variable, then we use it
  ...
  # the death date, and fit it with the actual death date.The goodness of fit is represented
  # by the Pearson statistic(using "cal_P" function), and this process is repeated 100 times.*

  # **If using bootstrapping, at this step, we will use the actual daily death victims data as
  # the parameter for the Poisson simulation. A single number will be generated in the place of 
  # real daily number of victims, which will then enter the next loop.
  ...
  for (j in 1:n.rep) {
    ...
    #     will be 5. The occurrence of 311 will not be returned by "tabulate".
    ...
    # The reason is that as the number of iterations increases, the fit of the expected death time 
    # improves, and the required step size for adjustments will decrease accordingly.

    d_real <- deaths_t # "d_real" means the real number of victims for each day. The length is 310.
    # "deaths_t" stores number of real death data from day 1-130. If "bs == FALSE", we use the real 
    # data to calculate the Pearson statistic.
    
    

    # Step 3.1*:
    # When we use bootstrapping, which means "bs == TRUE" as function input, we need to replace the
    # actual death data. The method is to use the actual death count for each day as the expected 
    # value for the Poisson simulation.
    ...
    if(bs){
      ...
      for(i in t){  
        ...
        # "t" and "deaths" correspond to recording the timr and the number of deaths, repectively.
        # "for(i in t)": "i" will automatically start from the first element, and it corresponds to
        # the first element of "deaths" as well.
    ...
    original_t0 <- t0 # The following code will change "t0", so store the "original t0" beforhand.
    ...
    #To avoid local overfitting, process "t0" in a random. "shuffle" refers to a random sequence.



    # Step 3.2*:
    # In this part, we need to recalculate the estimaed death date with different steps. Then, we 
    # update the vector of deaths per day. Finally, by comparing the results, we update the "t0",
    # the vector of deaths per day and P_value that produce the smaller Pearson statistic.
    for (i in shuffle){
          
      tmpt_d_simulation <- d_simulation 
      # Temporary variable, decied whether to keep it, based on Pearson statistic.
      ...
      # "tmpt_d_simulation" is the vector stored deaths per day, After moving step, the victim's
      # death date is updated from old date to a new one. So the count in old date's position
      #should be decreased by one, and new date's position should be increased by one.
      ...
      if (tmpt_new_esdeath_date < 1){
        ...
        # This is beacause we dont have real death data before day 1.  
      ...
      if (tmpt_new_esdeath_date > 310){
        ...
        # This is beacause we dont have real death data after day 310.  
        # And to ensure no errors occur. Discard in advance and jump into next loop.
      }
      
      tmpt_d_simulation[tmpt_new_esdeath_date] <- tmpt_d_simulation[tmpt_new_esdeath_date] + 1
          
      tmpt_P_value <- cal_P(d_real, tmpt_d_simulation) # the Pearson statistic afer adjusting
    ...
    # *It is worth noting that this plotting occurs within the loop,, so each time a plot is 
    # generated, the changes can be clearly observed.*
    if (bs){
      layout(matrix(c(1, 2), 1, 2)) # Generate a 1x2 layout for displaying plots.

      # Fig1 showing estimated deaths and new infection each day;simulated deaths after bootstrapping
      ...
      # The 2nd vector input defines the data points for each points on the x-axis. In this grap,
      # we draw a line showing current simulated deaths per day.
      ...
      legend("topright",legend = c("Estimate Death", "simulate Death", "Convergent Incidence "),  
  ...
  return(list(P = P_output, inft = inft_output, t0 = t0))
...
# Since they are very "dense", we Choose "black" with a transparency of 0.05
# to show them. 
